...
**Recommended Metigation:** There are few possible metigations:

1. Use a newer version of solidity, and a `uint256` instead of `uint64` for `PuppyRaffle::totalFees()`
2. You could also use the `SafeMath` Library of OPenZepplin for version 0.7.6 of solidity, However you would still have a hard time with `uint64` type if too many fees are collected.
...
There are more attack vectors with that finale require, so we recommend removing it regardless.



### [M-1] Looping through the array to check for duplicates in `PuppyRaffle::enterRaffle` is a potential denail of service (Dos) attack, incrementing gas cost for feature entrants.

**Description:** The `PuppyRaffle::enterRaffle()` function iterates over the `players` array to prevent duplicate entries. Because this check scales linearly with the array size, gas costs increase as more `players` join, eventually making the function uncallable and enabling a gas-based denial of service.
...
**Impact:** An attacker can prevent new participants from entering the raffle, effectively halting the protocol's core functionality. This deinal of service can persist indefinitely once triggered, resulting in loss of availability for all users. The raffle becomes unusable despite being correctly funded and deployed. 
...
1. Consider allowing duplicates. Users can make new wallet addresses anyways, so a duplicate check doesn't prevent the same person from entering multipile times, only the same wallet address.
...
### [L-1] `PuppyRaffle::getActivePlayer` returns 0 for non-existent players and for players at index 0, causing the players at index 0 to incorrectly think they have not entered the raffle.
...
**Impact** A player at index 0 may incorrectly thinks they have not entered the raffle, and attempt to enter the raffle again, wasting gas.
...
**Recommendations** The easiest recommendation is to revert if the player is not in the array, instead of returning 0.
